...
def show_dashboard():
    ...
    kpis = [
        ...
        (k3, "🔔", stats["active_alerts"], "Alertes actives" if lang=="fr" else "Active alerts", "#9A1515" if stats["active_alerts"] > 0 else "#4A5240"),
...
def show_dashboard(dark, paper, text, sub, brd, lang):
    ...
    # ── Recent predictions ────────────────────────────────────────────
    st.markdown("---")
    st.markdown(f"<div class='form-label-custom'>{'🕒 10 dernières prédictions Proxy PM2.5' if lang=='fr' else '🕒 Last 10 Proxy PM2.5 predictions'}</div>", unsafe_allow_html=True)
    preds = get_all_predictions_aqi(limit=10)
    if preds:
        df = pd.DataFrame(preds)
        cols_show = [c for c in ["city","region","date_pred","score","risk_level","username","created_at"] if c in df.columns]
        df = df[cols_show]
        col_map = {"city":"Ville","region":"Région","date_pred":"Date","score":"Proxy PM2.5",
                   "risk_level":"Niveau","username":"Utilisateur","created_at":"Enregistré le"}
...
def _show_aqi_map_plotly(dark, paper, text, sub, brd, lang):
    """Carte Proxy PM2.5 sobre via Plotly ScatterGeo."""
    scores = _mock_city_scores()
    lats, lons, names, vals, colors, hovers = [], [], [], [], [], []
    for city, d in scores.items():
        s = d["score"]
        lats.append(d["lat"]); lons.append(d["lon"]); names.append(city); vals.append(s)
        colors.append("#007A5E" if s<=33 else ("#B8860B" if s<=66 else "#B91C1C"))
        hovers.append(f"<b>{city}</b><br>{d['region']}<br>Proxy PM2.5: {s:.1f}/100<br>{'DANGER' if s>66 else ('VIGILANCE' if s>33 else 'SAFE')}")

    fig = go.Figure()
    for lvl, lc, lb in [("SAFE","#007A5E","SAFE (0–33)"),
                          ("VIGILANCE","#B8860B","VIGILANCE (34–66)"),
                          ("DANGER","#B91C1C","DANGER (67–100)")]:
        mask = [i for i,v in enumerate(vals) if
                (lvl=="SAFE" and v<=33) or (lvl=="VIGILANCE" and 33<v<=66) or (lvl=="DANGER" and v>66)]
        if mask:
            fig.add_trace(go.Scattergeo(
                lat=[lats[i] for i in mask], lon=[lons[i] for i in mask],
                mode="markers",
                marker=dict(size=[max(8, vals[i]/5) for i in mask], color=lc, opacity=0.85,
                            line=dict(color="#F7F5F0" if not dark else "#1C1C1E", width=1)),
                hovertext=[hovers[i] for i in mask], hoverinfo="text",
                name=lb,
            ))

    fig.update_layout(
        geo=dict(scope="africa", center=dict(lat=5.5, lon=12.3), projection_scale=8,
...
def _show_region_bars(dark, paper, text, brd, lang):
    ...
    colors = [get_risk_color("DANGER" if s>66 else ("VIGILANCE" if s>33 else "SAFE")) for s in scores]
...
def _show_evolution(dark, paper, text, brd, lang):
    ...
    kaki_colors  = {"Centre":"#6B7355","Nord":"#B8860B","Littoral":"#4A7A6A","Extreme-Nord":"#9A4A1A"}
